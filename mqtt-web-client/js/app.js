...
//        console.log("onMessageArrived:" + message.payloadString + " qos: " + message.qos);
    ...
    'render': {
        ...
        'message': function (message) {
            ...
            /* CONTROL AUTO MODE */
            let str_ret = message.payload;            
            
            if (str_ret.localeCompare("LED ON") == 0)
                document.getElementById("led").textContent = "ON";
            else if (str_ret.localeCompare("LED OFF") == 0)
                document.getElementById("led").textContent = "OFF";
            else if (str_ret.localeCompare("DEVICE OPEN") == 0)
                document.getElementById("device").textContent = "OPEN";
            else if (str_ret.localeCompare("DEVICE CLOSE") == 0)
                document.getElementById("device").textContent = "CLOSE";
            else if (str_ret.localeCompare("MODE AUTO") == 0)
                document.getElementById("mode_ctrl").textContent = "AUTO";
            else if (str_ret.localeCompare("MODE MANUAL") == 0) {
                document.getElementById("mode_ctrl").textContent = "MANUAL";
                websocketclient.publish("CONTROL","OVER MILESTONE",parseInt($('#publishQoSInput').val(),10),$('#publishRetain').is(':checked'));
            }
            else
            {
                let sub_str = message.payload.split(' ');

                if (sub_str[0].localeCompare("TEMPERATURE") == 0)
                    document.getElementById("tempp").textContent = sub_str[1];

                if (sub_str[3].localeCompare("HUMIDITY") == 0)
                    document.getElementById("humii").textContent = sub_str[4];

                /* Auto control device */
                let mode = document.getElementById("mode_ctrl").textContent;
                if (mode.localeCompare("AUTO") == 0)
                {
                    if (parseInt(sub_str[1]) > 40 || parseInt(sub_str[4]) > 65)
                    {
                        websocketclient.publish($("CONTROL").val(),$("OVER MILESTONE").val(),parseInt($('#publishQoSInput').val(),10),$('#publishRetain').is(':checked'))
                    }
                }
